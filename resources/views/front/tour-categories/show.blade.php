        ...
        <div class="row">
            <div class="col-md-12">
                <h1 class="bira" style="display: flex; align-items: center">
                    @if($category->icon)
                        <img src="images/tour_category_icons/{{ $category->icon }}" class="mr-1" alt="{{ $category->name }}">
                    @else
                        <i class="fa fa-folder-open-o text-danger mr-1"></i>
                    @endif
                    <span>
                        {{ $category->name }}
                        @if($category->tours->count()) <span class="badge assistant-light">{{ $category->tours->count().Str::plural(' tour',$category->tours->count()) }}</span> @endif
                    </span>
                </h1>
                <hr>
            </div>
