...
@section('content')
    ...
    <div class="container main">
        <div class="row">
                                ...
                                <div class="panel panel-default panel-raised panel-noroundcorners">
                                    ...
                                    <div class="panel-footer">
                                        ...
                                        @if(!is_null($tour->meta_description))
                                            <p class="text-justify assistant-light">{{ Str::limit($tour->meta_description, 150) }}</p>
                                            <hr>
                                        @endif
                                        ...
                                        @foreach($tour_categories->sortByDesc(function ($tour_category) {return $tour_category->tours->count();})->take(4) as $tour_category)
                                            ...
                                            <li>
                                                <h4 class="ubuntucondensed">
                                                    ...
                                                    <small class="assistant-light"> - {{ $tour_category->tours->count() }} {{ Str::plural('tour',$tour_category->tours->count()) }}</small>
                                                </h4>
                                                <p>{!! Str::limit($tour_category->meta_description, 140) !!}</p>
                            ...
                            <div class="panel panel-default random-category panel-noroundcorners">
                                ...
                                <div class="panel-body p-0" style="overflow: hidden">
                                    <a href="/safaris/{{ $random_category->slug }}" title="{{ $random_category->name }}">
                                        <img src="images/tour_category_images/{{ $random_category->photo }}" class="img-responsive" alt="{{ $random_category->name }}">
                                    </a>
                                </div>
                                <div class="panel-footer">
                                    <h3 class="mt-2 mb-3">
            ...
            @endif
            @if($photos->count())
                <div class="col-md-12 home-galler-section">
                    <hr class="hr-text bira" data-content="Tanzania Tourism Photos">
                    <div class="home-gallery-container">
                        @foreach($photos->sortByDesc('id')->take(10) as $photo)
                            <div class="image-container">
                                <a href="/tour/gallery" title="{{ $photo->name }} - View the gallery!">
                                    <img src="images/gallery_photos/{{ $photo->photo }}" alt="{{ $photo->name }}">
                                </a>
                            </div>
                        @endforeach
                    </div>
{{--                    <div class="home-gallery-overlay text-center"></div>--}}
                </div>
            @endif
        </div>
